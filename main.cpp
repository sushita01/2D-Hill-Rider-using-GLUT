...
void drawHUD()
{
    ...
    // =================================================
    // IN-GAME VANISHING MILESTONE POPUP
    // -------------------------------------------------
    // Only drawn while bonusDisplayTimer > 0; that timer
    // counts down every frame in timer() and is set to a
    // short duration (1.5s) whenever a milestone that
    // wants a big popup is reached (coin 100/300, or every
    // 1000m distance milestone) - see checkCoinMilestones()
    // / checkDistanceMilestones(). bonusDisplayText and the
    // bonusDisplayR/G/B color are set alongside the timer,
    // so this single block can show any milestone message.
    // Positioned dead-center of the screen, in the open sky
    // roughly above the car, well clear of the road, using
    // the same scalable stroke font as the Game Over title
    // so it reads as a big, bold banner.
    // =================================================
